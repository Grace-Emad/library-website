...
function loadBookDetails() {
    ...
    if (!selectedBookIndex) {
        document.getElementById('book-details-container').innerHTML = `
            <div>
                ...
                <button onclick="window.location.href='search_books.html'">Back to Search</button>
            </div>
        `;
        return;
    }
    ...
    document.getElementById('book-details-container').innerHTML = `
        <div>
            ...
            <p><strong>Status:</strong> <span id="status-value">${book.status === 'available' ? 'Available' : 'Not Available (Borrowed)'}</span></p>
        </div>
    `;
...
function updateBorrowButton(isAvailable) {
    ...
    if (isAvailable) {
        borrowSection.innerHTML = `<button id="borrowBtn">Borrow This Book</button>`;
        document.getElementById('borrowBtn').addEventListener('click', borrowCurrentBook);
    } else {
        borrowSection.innerHTML = `<button disabled>Not Available for Borrowing</button>`;
    }
}
    ...
    if (!user || !user.borrowedBooks || user.borrowedBooks.length === 0) {
        document.getElementById('borrowed-list-container').innerHTML = `
            <div>
                ...
                <button onclick="window.location.href='search_books.html'">Browse Books</button>
            </div>
        `;
        return;
    }
